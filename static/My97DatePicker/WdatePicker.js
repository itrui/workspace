...
(function () {
    var $ = {
        ...
        $skinList: [
            ...
            {name: "zw_def", charset: "gb2312"},
            {name: "zw_qs", charset: "gb2312"},
    ...
    function I(G, _) {
        ...
        if (!$dp.dd || G.eCont || ($dp.dd && (G.getRealLang().name != $dp.dd.lang || G.skin != $dp.dd.skin))) {
            if (G.eCont)E(G.eCont, G); else {
                ...
                $dp.dd.style.cssText = "position:absolute;-webkit-box-shadow: 1px 1px 5px #666;-moz-box-shadow: 1px 1px 5px #666;box-shadow: 1px 1px 5px #666;background:#fff;";
        ...
        function E(K, J) {
            ...
            function C() {
                ...
                K.lang = _.name;
                K.skin = J.skin;
                var $ = ["<head><script>", "", "var doc=document, $d, $dp, $cfg=doc.cfg, $pdp = parent.$dp, $dt, $tdt, $sdt, $lastInput, $IE=$pdp.ie, $FF = $pdp.ff,$OPERA=$pdp.opera, $ny, $cMark = false;", "if($cfg.eCont){$dp = {};for(var p in $pdp)$dp[p]=$pdp[p];}else{$dp=$pdp;};for(var p in $cfg){$dp[p]=$cfg[p];}", "</script><script src=", A, "lang/", _.name, ".js charset=", _.charset, "></script>"];
